...
def check_plagiarism():
    data = request.get_json()
    text = data.get('text')
    response = requests.post('https://api.plagiarismchecker.com/v1/check', json={'text': text}, headers={'Authorization': 'Bearer YOUR_API_KEY'})
    if response.status_code == 200:
        return jsonify(response.json())
    else:
        return jsonify({'error': 'Unable to check plagiarism'}), 500
