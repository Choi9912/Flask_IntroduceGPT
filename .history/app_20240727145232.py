...
class PlagiarismChecker:
    ...
    def check(self, text):
        ...
        try:
            response = requests.post(self.api_url, json=messages, headers=headers)
            response.raise_for_status()
            logging.debug(f"API response: {response.json()}")  # 디버그를 위해 API 응답 출력
    ...
    def parse_result(self, result):
        ...
        if 'plagiarism_percentage' in result and 'sources' in result:
            plagiarism_details = {
            ...
            return 'No plagiarism details returned by API'
