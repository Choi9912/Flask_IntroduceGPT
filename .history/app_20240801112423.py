...
class SelfIntroductionAnalyzer:
    ...
    def unified_prompt(self, introduction):
        role = "당신은 자기소개서를 분석하는 AI입니다."
        user_content = f"다음 자기소개서를 분석해주세요: {introduction} 자기소개서를 분석하여 주요 포인트와 주제를 식별하고, 구조와 일관성을 평가합니다. 자기소개서의 강점과 개선할 점을 자세하게 알려주세요."
...
class SelfIntroductionWriter:
    ...
    def unified_prompt(self, prompt):
        role = "당신은 자기소개서를 작성하는 AI입니다."
        user_content = f"다음 프롬프트로 자기소개서를 작성해주세요: {prompt} 소제목을 포함하고, 문단을 나누어 500자 이상으로 자기소개서를 작성해주세요."
        return send_api_request(self.api_url, role, user_content)
...
class PlagiarismDetector:
    ...
    def unified_prompt(self, text):
        role = "당신은 텍스트를 분석하여 표절 여부를 판단하는 AI입니다."
        user_content = f"다음 텍스트의 표절 여부를 검사해주세요: {text} 텍스트를 분석하여 주요 포인트와 주제를 식별하고, 문장 구조와 단어 선택을 평가합니다. 아이디어의 독창성을 평가하고, 다른 출처와의 유사성을 평가하여 표절 여부를 판단해주세요."
        return send_api_request(self.api_url, role, user_content)
...
class SpellChecker:
    ...
    def unified_prompt(self, text):
        role = "당신은 맞춤법 검사 AI입니다."
        user_content = f"다음 글의 맞춤법을 확인해주세요: {text} 텍스트의 철자와 문법을 철저히 검사하고, 문장 구조와 일관성을 고려하여 전체 텍스트를 평가해주세요."
        return send_api_request(self.api_url, role, user_content)
...
class InterviewQuestionGenerator:
    ...
    def unified_prompt(self, job_description):
        role = "당신은 채용정보를 분석하여 면접 질문을 생성하는 AI입니다."
        user_content = f"다음 채용정보를 바탕으로 면접 질문을 생성해주세요: {job_description} 채용정보를 분석하여 주요 요구사항과 필수 스킬을 식별하고, 이에 맞는 면접 질문을 생성합니다. 채용정보에 명시된 요구사항과 필수 스킬을 기반으로 질문을 만들어 주세요."
        return send_api_request(self.api_url, role, user_content)
...
class PromptOptimizerApp:
    ...
    def generate(self):
        ...
        answer = self.writer.write(question, 'unified_prompt')
