...
class PlagiarismChecker:
    ...
    def check(self, text):
        messages = [
            ...
            {"role": "user", "content": f"다음 글의 표절 여부를 확인해주세요: {text}"}
        ]
        headers = {
            'Content-Type': 'application/json'
        }
        try:
            response = requests.post(self.api_url, json={"messages": messages}, headers=headers)
            response.raise_for_status()
    ...
    def parse_result(self, result):
        # Assuming the API returns detailed results with plagiarism percentage and matched sources
        choices = result.get('choices')
        if not choices:
            return 'No choices returned by API'
        
        content = choices[0]['message']['content']
        # Parsing the response content to extract details (mock implementation, adjust as per real response format)
        plagiarism_details = {
            "summary": content,
            "percentage": self.extract_percentage(content),
            "sources": self.extract_sources(content)
        }
        return plagiarism_details

    def extract_percentage(self, content):
        # Mock implementation, adjust according to the real response format
        import re
        match = re.search(r'표절 비율: (\d+)%', content)
        return int(match.group(1)) if match else None

    def extract_sources(self, content):
        # Mock implementation, adjust according to the real response format
        import re
