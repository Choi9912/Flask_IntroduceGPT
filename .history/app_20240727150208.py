...
class PlagiarismChecker:
    ...
    def check(self, text):
        try:
            ...
            text_parts = self.split_text_for_similarity_check(text)
    ...
    def identify_key_points(self, text):
        messages = [
            {"role": "system", "content": "당신은 카피킬러 AI입니다."},
    ...
    def split_text_for_similarity_check(self, text):
        messages = [
            {"role": "system", "content": "당신은 카피킬러 AI입니다."},
            {"role": "user", "content": f"다음 글을 유사성 검사를 위해 적절히 분할해주세요: {text}"}
        ]
        response = requests.post(self.api_url, json=messages)
        response.raise_for_status()
        return response.json()
